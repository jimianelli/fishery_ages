...
df_age <- df_age_big |> mutate(
  season=ifelse(month(HAUL_OFFLOAD_DATE)<6,"A", "B"),
  fmp=ifelse(NMFS_AREA<600,"BSAI","GOA")) |> select(YEAR,season,fmp, SEX, AGE, LENGTH)
```
...
plot_sex_ratio <- function(data, x_var, facet_var = NULL, min_n = 5,
  ...
  group_vars <- c(facet_vars, list(x_var))
  ...
  p <- ggplot(plot_df, aes(x = !!x_var, y = sex_ratio))
  ...
  if (show_labels) {
    ...
    p <- p +
      geom_point(aes(size = total_n), alpha = 0.4)
  }

  p <- p +
    geom_smooth(method = "loess", formula = "y ~ x", color = "darkred") +
...
plot_mean_length_age_sex <- function(data, age_range = 2:10, facet_ncol = 6,
  ...
  group_names <- unique(c(facet_names, facet_grid_names, "age_num", "sex", color_var, shape_var))
  ...
  line_group_names <- unique(c(color_var, shape_var, if (!facet_sex) "sex"))
...
The all-years pooled comparisons are shown separately for GOA in @fig-sex-ratio-overall-goa and for BSAI in @fig-sex-ratio-overall-bsai. The length-based trend by year for GOA is shown in @fig-sex-ratio-length-year-goa, the corresponding BSAI view is shown in @fig-sex-ratio-length-year-bsai, the age-based view by year is in @fig-sex-ratio-age-year, a younger-age-focused view is in @fig-sex-ratio-length-age, mean length-at-age by sex (ages 2-10) is shown in @fig-mean-length-age-sex, and corresponding views split by fishery management program are shown in @fig-mean-length-age-sex-fmp and @fig-mean-length-age-sex-season-fmp.
...
```{r}
#| label: fig-sex-ratio-age-year
#| fig-cap: "Sex ratio (proportion female) by age, faceted by year (YEAR > 2000)."
#| fig-width: 12
#| fig-height: 14

df_age %>%
  filter(YEAR > 2000) %>%
  plot_sex_ratio(x_var = AGE, facet_var = YEAR, show_labels = FALSE, facet_ncol = 5)
```

```{r}
#| label: fig-sex-ratio-length-age
#| fig-cap: "Sex ratio (proportion female) by length, faceted by age (AGE < 11)."
#| fig-width: 11
#| fig-height: 8

df_age %>%
  filter(AGE < 11) %>%
  plot_sex_ratio(x_var = LENGTH, facet_var = AGE, show_labels = FALSE, facet_ncol = 4)
```

```{r}
#| label: fig-mean-length-age-sex
#| fig-cap: "Mean length by age (2-10) using records with non-missing ages, with sex shown by color and fishery management program shown by symbol."
#| fig-width: 10
#| fig-height: 6
...
#| fig-cap: "Mean length by age (2-10) and sex using records with non-missing ages, faceted by fishery management program."
#| fig-width: 11
#| fig-height: 6

plot_mean_length_age_sex(df_age, facet_fmp = TRUE, facet_ncol = 2)
```
