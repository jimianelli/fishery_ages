...
#| include: false
#| cache: true
#| cache.extra: !expr file.info("df_age.csv")$mtime

df_age <- read_csv("df_age.csv", show_col_types = FALSE) |>
  mutate(
    season = factor(season, levels = c("A", "B")),
    fmp = factor(fmp, levels = c("BSAI", "GOA")),
    subarea = factor(subarea, levels = c("SE_BS", "NW_BS", "AI", "WGOA", "CGOA"))
  )
...
The all-years pooled Bering Sea summer acoustic-survey comparison is shown in @fig-acoustic-sex-ratio-overall-bsai, with age-based sex ratios on the top panel and 5-cm length-bin sex ratios on the bottom panel.
...
Year-specific Bering Sea summer acoustic-survey length-based sex ratios are shown in @fig-acoustic-sex-ratio-length-year-bsai.
...
The all-years pooled GOA winter acoustic-survey comparison is shown in @fig-acoustic-sex-ratio-overall-goa, with age-based sex ratios on the top panel and 5-cm length-bin sex ratios on the bottom panel.
...
Year-specific GOA winter acoustic-survey length-based sex ratios are shown in @fig-acoustic-sex-ratio-length-year-goa.
...
## Methods (for evaluating fishery data)
...
#| include: false
#| cache: true
#| cache.extra: !expr paste(file.info("df_age.csv")$mtime, as.character(packageVersion("brms")), as.character(packageVersion("cmdstanr")))
library(brms)
library(gt)
